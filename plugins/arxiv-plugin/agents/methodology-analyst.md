...
3. For papers with unclear methodology, fetch full metadata using WebFetch with `https://arxiv.org/api/query?id_list=<arxiv_id>` and prompt: "Extract the paper as JSON with fields: id, title, authors (array), abstract (COMPLETE text verbatim), published, categories (array), pdf_url, arxiv_url. Return ONLY the JSON object."
